...
public class ModifyFiles {
    ...
    public static void modify(Map<String, Map<String, Object>> taskMap) {
        final File tempDir = DataHelper.ext.behaviorTempDir;
            ...
            entry ->
                ...
                switch (type) {
                    ...
                    case Const.TY_JAR:
                        ...
                        outJar.renameTo(new File(DataHelper.ext.behaviorDir, outJar.getName()))
                        break;
                }
        })
    }
    ...
    public static void modifyAar(File targetFile, Map<String, Object> map) {
        final File behaviorDir = DataHelper.ext.behaviorDir;
        final File tempDir = DataHelper.ext.behaviorDirTempDir;
        ZipFile zipFile = new ZipFile(targetFile);
        Enumeration<ZipEntry> entries = zipFile.entries();

        def outputAar = new File(behaviorDir, targetFile.name)
        if (outputAar.exists()) {
            outputAar.delete()
        }
